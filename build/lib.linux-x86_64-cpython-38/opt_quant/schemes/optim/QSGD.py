...
class QSGD(Optimizer, QuantizeBase):
    r"""Implements quantized stochastic gradient descent (optionally with quantized momentum).
    ...
    def __init__(self, params, lr=required, momentum=0, dampening=0,
        ...
        Optimizer.__init__(self, params, defaults=defaults)

        QuantizeBase.__init__(self, representation=representation,
                                             rounding=rounding,
                                             bitwidth=bitwidth,
                                             **bitwidth_kwargs)

        assert scheme in ['absmax'], 'Optimizer quantization scheme should be in [abxmax]!'
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            for p, momentum_buffer in zip(params_with_grad, momentum_buffer_list):
                ...
                state = self.state[p]
                if self.q_scheme == 'absmax':
                    mxAbs = torch.max(torch.abs(momentum_buffer))
                    ...
                    if self.representation == 'rdx4':
                        ...
                        gradExp = torch.ceil(torch.log2(mxAbs))
                        mxExp = 2 ** (self.bitwidth_kwargs['sig'] - 1) - 1
                        curr_scale = 2 ** (gradExp - mxExp)
                    ...
                    quantized_ = self.quant_fun(momentum_buffer / scale, bits=self.bitwidth, **self.bitwidth_kwargs) * scale

                state['momentum_buffer'] = quantized_
