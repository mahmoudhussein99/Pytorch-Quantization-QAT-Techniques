...
class lsq_act(QuantizeBase):
    ...
    def __init__(self,
                 ...
                 per_channel=False,
                 **kwargs):
        super(lsq_act, self).__init__(representation=representation,
                                  rounding=rounding,
                                  bitwidth=bitwidth,
                                  **kwargs)
        assert mode in ['signed',
                        'unsigned'], 'LSQ-A: mode could be signed/unsigned!'
        self.mode = mode
        self.per_channel = per_channel
        self.s = Parameter(torch.tensor(float(1.0)))
        self.first_time_call = True
        self.kwargs = kwargs
        assert self.representation == 'int', 'LSQ: unsiged mode only supports int!'

        if mode == 'unsigned':
            self.thd_neg = 0
            self.thd_pos = 2**self.bitwidth - 1
        else:
