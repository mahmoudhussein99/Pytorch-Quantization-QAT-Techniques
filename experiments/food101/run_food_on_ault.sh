...
#  sbatch job_ault.sbatch  "python3  food_train_eval.py  --seed ${SEED} --model resnet50"
#done
...
for SEED in "${seeds[@]}"; do
  for AS in "${act_schemes[@]}"; do
    for WS in "${weight_schemes[@]}"; do
          sbatch job_ault.sbatch "python3 food_train_eval.py --lr 0.01 --seed ${SEED} --act_qmode ${AS} --act_bits 8  --weight_qmode ${WS} --weight_bits 8  --shortcut_quant False --model ${ARCH}"
            done done done
