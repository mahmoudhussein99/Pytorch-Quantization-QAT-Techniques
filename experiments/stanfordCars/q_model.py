...
class BasicBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        out = self.add_op(out, self.shortcut(x))
        out = self.act_fn_2(out)
        return out

class ResNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes=196,
...
def MobileNetV2(w_qmodule=nn.Identity(), act_qmodule=nn.Identity(), err_qmodule=nn.Identity(),
        bn_w_qmodule=nn.Identity(), bn_act_qmodule=nn.Identity(), bn_err_qmodule=nn.Identity(),
        first_w_qmodule=nn.Identity(), first_act_qmodule=nn.Identity(), first_err_qmodule=nn.Identity(),
        **kwargs):
